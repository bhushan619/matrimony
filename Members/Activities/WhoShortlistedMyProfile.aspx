...
<a href="Notification.aspx"> <i class="fa fa-bell-o fa-2x iconcolor"></i>
   ...
   <div class="collapse navbar-collapse" id="example-navbar-collapse">
	...
	<div class="container">		      
               <div class="col-lg-8">
    <h3>  Members Shortlisted  My Profile </h3>    <div class="borsolid"></div>  
                   <span class="j10font"> The following  members have shortlisted your profile.</span> 
                   <br />
   ...
   <div class="col-lg-8 "> 
       ...
       <div class="jsideright">Shortlisted On: <asp:Label ID="Label7" runat="server" Text='<%# Eval("SlstMemDate") %>' /> &nbsp;&nbsp;  
...
<asp:ImageButton ID="imgSimilarPic" runat="server"   CssClass="img-thumbnail" Height="100px" Width="100px"  OnClick="imgmember_Click"  CommandArgument='<%# Eval("SlstMemId") +";"+ Eval("SlstMemPhoto")  %>'   ImageUrl='<%# "~/members/media/" + Eval("SlstMemPhoto") %>' /> 
         </div>
<div class="col-lg-8" >
<%-- Name:Profile Created For <asp:Label ID="Label9" runat="server" Text='<%# Eval("SlstMemAccFor") %>' />,<br /> --%>
    <asp:LinkButton ID="Label10" runat="server" Text='<%# Eval("SlstMemName") %>' OnClick="OpenProfile"    CommandArgument='<%# Eval("SlstMemId") %>'  Font-Bold="true" Font-Size="17px"/>  
    (<asp:LinkButton ID="lblCollege" runat="server"    CommandArgument='<%# Eval("SlstMemId") %>'    OnClick="OpenProfile" Text='<%# Eval("SlstMemId") %>' Font-Bold="true" Font-Size="17px"></asp:LinkButton> )
&nbsp;&nbsp; <asp:LinkButton ID="lblname" runat="server"    CommandArgument='<%# Eval("SlstMemId") %>'  CssClass="label label-success"  OnClick="OpenProfile" Text='<%# Eval("SlstMemPackage") %>'></asp:LinkButton> 
      <br /><br />
<asp:Label ID="AgeLabel" runat="server" Text='<%# Eval("SlstMemAge") %>' />,
   <asp:Label ID="HeightLabel" runat="server" Text='<%# Eval("SlstMemHeight") %>' /> / 
           <asp:Label ID="Label6" runat="server" Text='<%# Eval("SlstMemHeightcm") %>' /> | 
<b>Religion : </b>   <asp:Label ID="ReligionLabel" runat="server" Text='<%# Eval("SlstMemReligion") %>' />,
 Caste :     <asp:Label ID="CasteLabel" runat="server" Text='<%# Eval("SlstMemCaste") %>' /> ,
 Subcaste :     <asp:Label ID="Label5" runat="server" Text='<%# Eval("SlstMemSCaste") %>' /> |<br />
 <b> Location : </b> <asp:Label ID="Label3" runat="server" Text='<%# Eval("SlstMemCity") %>' />,  
       <asp:Label ID="Label2" runat="server" Text='<%# Eval("SlstMemState") %>' /> ,
         <asp:Label ID="CityLabel" runat="server" Text='<%# Eval("SlstMemCountry") %>' /> |
<b> Education :</b>    <asp:Label ID="EducationLabel" runat="server" Text='<%# Eval("SlstMemEducation") %>' /> |<br />
  <b>  Occupation : </b>        <asp:Label ID="Label4" runat="server" Text='<%# Eval("SlstMemOccupation") %>' />  <br />
   <br />  <asp:LinkButton ID="LinkButton1" runat="server"    CommandArgument='<%# Eval("SlstMemId") %>'  OnClick="OpenProfile" Text="View Full Profile"></asp:LinkButton>
   <%-- About <asp:Label ID="Label11" runat="server" Text='<%# Eval("SlstMemAccFor") %>' />: 
     <asp:Label ID="lblDescription" runat="server"  Text='<%# Limit(Eval("SlstMemAbout"),40) %>'   Tooltip='<%# Eval("SlstMemAbout") %>'>      </asp:Label>
      <asp:LinkButton ID="ReadMoreLinkButton" runat="server"  Text="Read More"  CommandArgument='<%# Eval("SlstMemId") %>' OnClick="OpenProfile">      </asp:LinkButton>--%>
 </div>    
             </div>     
         </div>  
                  <div class="row " >
   <div class="bordex"></div>  
        ...
        <asp:LinkButton ID="lnkShortlist" runat="server" onclick="lnkShortlist_Click"  CommandArgument='<%# Eval("SlstMemId") %>' Text="Shortlist" class="btn btn-warning btn-xs"></asp:LinkButton>     
               </div>
      </div> </div>
          </ItemTemplate>
